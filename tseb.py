...
**Modifications**:
    ...
    The user can download the geeet package available at : https://github.com/kaust-halo/geeet
    The user can then replace the TSEB module with this code; to use with the GEE TSEB workflow.
    The package with modifications is also available at : https://osf.io/mkpts/?view_only=a9b51cf594fd4ce3a2d1b4a73f4037a1
"""
"""
...
def tseb_series(img=None,    # ee.Image with inputs as bands (takes precedence over numpy arrays)
    ...
    """
    from geeet.vegetation import compute_lai, compute_ftheta, compute_Rns,compute_fapar,compute_fipar,compute_fg
    ...
    if is_img(img):
        ...
        dewpoint_temperature = img.select('dewpoint_temperature_2m') # in K
        #RH = img.select('relative_humidity') # in %
        ...
        # Optional
        LAI_neural = img.select('LAI_neural')  # TODO: check if band exists
   ...
   # LAI = compute_lai(NDVI, k_par) # TODO: check if LAI is given as an input and skip this compute_lai.
    ...
    f_theta = compute_ftheta(LAI_neural, theta=Vza) # Fraction of field of view of sensor occupied by canopy (e.g. ~0 for LAI=0; ~0.4 for LAI=1, ~1 for LAI>6 with theta=0)
   ...
   # CH = init_canopy(LAI, CH, min_LAI, D_0_min) # Initializes canopy height based on LAI  
    ...
    Rns = compute_Rns(Rn, LAI_neural, solar_angles, use_zenith=True, k=k_rns) # Soil net radiation Rns = E*Rn where E is either just exp(-kLAI)
    ...
    met_params = compute_met_params(Ta, P, dewpoint_temperature) # computes the following meteorological parameters:
   ...
   # F_g = compute_fg(NDVI)
    ...
    resist = RN95(U, CH, rough, LAI_neural, Leaf_width, zU, zT, Ustar = ustar, L=1e10) # computes three resistances: Ra, Rs, and Rx (see geeet.resistances.RN95)
    ...
    if is_img(img):
        ...
        RH = met_params.select('RH')
        ra = resist.select('Ra')
        ra = ra.max(0.01).min(5000)
        rs = resist.select('Rs')
        rs = rs.max(0.01).min(5000)
        rx = resist.select('Rx')
        rx = rx.max(0.01).min(5000)
       ...
       # LEc = taylor.multiply(Rnc).multiply(AlphaPT*F_g) # Canopy latent heat flux (N95 equation 12)
        LEc = taylor.multiply(Rnc).multiply(AlphaPT).multiply(F_g) # Canopy latent heat flux (N95 equation 12)
    #   LEc = Rnc.multiply(AlphaPT*F_g).multiply(taylor)
        LEc = Rnc.multiply(AlphaPT).multiply(F_g).multiply(taylor)
        ...
        initialImg = Tc.addBands(Ts).addBands(Tac).addBands(Hc).addBands(Hs).addBands(LEc).addBands(LEs).addBands(RH)\
        .addBands(ra).addBands(rs).addBands(rx).addBands(ustar).addBands(Image(AlphaPT)).addBands(Image(0))
        initialImg = initialImg.rename('Tc','Ts','Tac','Hc','Hs','LEc','LEs','RH','Ra','Rs','Rx',\
            'Ustar','alphaPT','iteration')
            # initialImg = Tc.addBands(Ts).addBands(Tac).addBands(Hc).addBands(Hs).addBands(LEc).addBands(LEs).addBands(F_g)\
            # .addBands(ra).addBands(rs).addBands(rx).addBands(ustar).addBands(Image(AlphaPT)).addBands(Image(0))
            # initialImg = initialImg.rename('Tc','Ts','Tac','Hc','Hs','LEc','LEs','Fg','Ra','Rs','Rx',\
            #     'Ustar','alphaPT','iteration')
        ...
        def tseb_series_iteration(img, list):
            ...
            rao = rao.max(0.01).min(5000)
            rso = oldImg.select('Rs')
            rso = rso.max(0.01).min(5000)
            rxo = oldImg.select('Rx')
            rxo = rxo.max(0.01).min(5000)
            ...
            resistU = RN95(U, CH, rough, LAI_neural, Leaf_width, zU, zT, Ustar = ustaru, L=Lu)  # Ustar: new; L: new
            rau = resistU.select('Ra')
            rau = rau.max(0.01).min(5000)
            rsu = resistU.select('Rs')
            rsu = rsu.max(0.01).min(5000)
            rxu = resistU.select('Rx')
            rxu = rxu.max(0.01).min(5000)
            ...
            updatedImg = Tcu.addBands(Tsu).addBands(Tacu)\
                .addBands(Hcu).addBands(Hsu).addBands(LEcu).addBands(LEsu).addBands(RH)\
            ...
            updatedImg = updatedImg.rename('Tc','Ts','Tac','Hc','Hs',\
                'LEc','LEs','RH','Ra','Rs','Rx','Ustar','alphaPT','iteration')
               # updatedImg = Tcu.addBands(Tsu).addBands(Tacu)\
               #     .addBands(Hcu).addBands(Hsu).addBands(LEcu).addBands(LEsu).addBands(F_g)\
               #     .addBands(rau).addBands(rsu).addBands(rxu)\
               #     .addBands(ustaru).addBands(AlphaPTu).addBands(iterationu)
               # updatedImg = updatedImg.rename('Tc','Ts','Tac','Hc','Hs',\
               #     'LEc','LEs','Fg','Ra','Rs','Rx','Ustar','alphaPT','iteration')
        ...
        # resultImage = resultImage.addBands(G.rename('G')).addBands(Rn.rename('Rn')).addBands(F_g.rename('Fg'))\
        #     .addBands(Rns.rename('Rns')).addBands(Rnc.rename('Rnc'))
        ...
        for iteration in range(max_iterations):
            ...
            ### Update resistances
            resistu = RN95(U, CH, rough, LAI_neural, Leaf_width, zU, zT, Ustar=ustar, L=L)
            rau, rsu, rxu = resistu
            rau = rau.max(0.01).min(5000)
            rsu = rsu.max(0.01).min(5000)
            rxu = rxu.max(0.01).min(5000)
        ...
        et_tseb_out = LE, LEs, LEc, Hs, Hc, G, Rn, Rns, Rnc, Ts, Tc, Tac, ra, rs, rx, it, LAI_neural, CH,RH
        et_tseb_out_keys = ['LE', 'LEs', 'LEc', 'Hs', 'Hc', 'G', 'Rn', 'Rns', 'Rnc', 'Ts', 'Tc', 'Tac', 'Ra', 'Rs', 'Rx', 'iteration', 'LAI_neural','canopy_height','RH']
